...
Board.prototype.sendUpdate = function (socket) {
  ...
  this.players.forEach(function (player) {
    self.io.sockets.socket(player.id).emit('updateBoard', { board: self.uuid, players : [ { cards: [self.players[0].hand[0].toURL(), self.players[0].hand[1].toURL()] } ], dealer: {cards: [self.dealer.hand[0].toURL()]} } );
  });
}
