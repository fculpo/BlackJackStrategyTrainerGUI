...
Board.prototype.sendUpdate = function (socket) {
  ...
  this.players.forEach(function (player) {
    self.io.sockets.socket(player.id).emit('updateBoard', { board: self.uuid, players : [ { strategyscore: self.players[0].scoreStrategy(), score: self.players[0].score(), cards: [self.players[0].hand[0].toURL(), self.players[0].hand[1].toURL()] } ], dealer: {cards: [self.dealer.hand[0].toURL()]} } );
  });
}
