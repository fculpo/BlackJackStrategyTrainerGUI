  ...
  if (this.config.howManyPlayersMax > this.playersNb) {
    if ((pos > 0) && (pos <= this.config.howManyPlayersMax)) {
      ...
      if (freePos === true) {
        ...
        this.playersNb++;
        logger.info("Successfully added Player " + id + " at position " + pos);
        cb("ok");
      } else {
        logger.info("Requested position " + pos + " is already taken !");
      }
    } else {
      logger.info("Invalid Position asked ! Max Players = " + this.config.howManyPlayersMax);
    }
  } else {
    logger.info("Maximum number of players is reached !");
  }
}
...
Board.prototype.dealHandNoBJ = function () {
  ...
  for (var key in this.players) {
    if (this.players.hasOwnProperty(key)) {
      logger.info(this.players[key].hand);
      logger.info("SCORE: " + this.players[key].scoreStrategy());
