...
BJT.prototype.start = function () {
  ...
  this.app.io.configure(function () {
    self.app.io.set('log level', 2);
    self.app.io.set("transports", ["xhr-polling"]);
    self.app.io.set("polling duration", 10);
  });
  ...
  for (var key in this.app.boards) {
    if (this.app.boards.hasOwnProperty(key)) {
      if (this.app.boards[key].playersNb < this.app.boards[key].config.howManyPlayersMax) {
        ...
        console.log(this.app.boards[key].playersNb + "/" + this.app.boards[key].config.howManyPlayersMax + " players");
